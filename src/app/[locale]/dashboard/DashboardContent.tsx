            ...
            <div className="space-y-8">
              ...
              <AnimatePresence>
                {enrollmentsByChild.map((group, gIdx) => {
                  const agePart = typeof group.childAge === "number" ? t("childAgePart", { age: group.childAge }) : "";
                  const gradePart = group.gradeLevel ? t("childGradePart", { grade: group.gradeLevel }) : "";
                  return (
                    <motion.div key={group.key} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.28 + gIdx * 0.06 }} className="space-y-3">
                      <div className="flex flex-wrap items-baseline justify-between gap-2 px-0.5">
                        <h3 className="text-base font-semibold">{t("childMeta", { name: group.childName, agePart, gradePart })}</h3>
                        <span className="text-xs text-muted">{t("enrollmentsForChild", { count: group.enrollments.length })}</span>
                      </div>
