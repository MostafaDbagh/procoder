...
export default function CoursesPage() {
  ...
  const filtered = useMemo(() => {
    return courses.filter((c) => {
      ...
      if (search) {
        const title = ct(c.titleKey).toLowerCase();
        if (!title.includes(search.toLowerCase())) return false;
      }
      return true;
    });
  }, [category, age, level, search, ct]);
          ...
          >
            <div className="relative">
              ...
              {search && (
                <button
                  onClick={() => setSearch("")}
                  className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 rounded-lg text-muted hover:text-foreground hover:bg-surface-hover transition-colors"
                >
                  <X className="w-4 h-4" />
                </button>
              )}
          ...
          >
            ...
            {hasFilters && (
              <button
                ...
                className="ml-auto flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-red-500 bg-red-50 dark:bg-red-950/30 hover:bg-red-100 dark:hover:bg-red-950/50 transition-colors"
              >
                <X className="w-3 h-3" />
                Clear all
              </button>
          ...
          <AnimatePresence mode="wait">
            {filtered.length > 0 ? (
              ...
              >
                ...
                <button
                  onClick={clearFilters}
                  className="mt-3 px-5 py-2.5 rounded-xl bg-primary text-white text-sm font-semibold hover:scale-[1.02] transition-transform"
                >
                  Clear filters
                </button>
              </motion.div>
